...
- **版面設計**：依據品牌構圖重新設計
  - 背景改為黑色 (#000000)
  - 頂部使用 `banner.jpg` 作為圖片區塊
  - 桌面模式：圖片以 contain 顯示完整原圖，限制大小（max-width 600px, max-height 30vh）
  - 手機模式：max-height 30vh
- **內容區塊**：Logo（速度計弧形、跑車剪影、YR JOURNAL）、商店標題「1/64模型車專賣店」、社交媒體連結
- **內容卡片**：半透明背景、圓角、模糊效果，整合商店資訊與社群連結
- **響應式**：支援手機、平板、桌面不同螢幕

#### 會員登入按鈕 (`src/views/orderpage/Add.vue` activator)
- 按鈕樣式調整：深藍色 (#1E3A8A)、圓角 12px、陰影、hover 時輕微上浮

#### 會員登入彈窗 (`src/views/orderpage/Add.vue`)
- **深色主題**：對話框與主頁面黑色主題一致
- **樣式調整**：標題欄、輸入框、確認按鈕、會員資訊卡片、消費記錄表格
- **配色優化**：提升文字對比度，改善可讀性（背景 #2a2a2a、輸入框半透明、純白文字、文字陰影）
- **會員資料**：新增 ID 欄位顯示於左側（顯示 snkey 或 id）
